...
namespace Zombie
{
    public class GameFacade
    {
        ...
        public void Init()
        {
            mCampSystem = new CampSystem(); //兵营系统,生产植物
            mCharacterSystem = new CharacterSystem();  //角色管理系统 管理植物和僵尸
            mGameEventSystem = new SubscribeSystem();  //收集数据
            mStageSystem = new StageSystem(); //责任链关卡系统 生产僵尸
            mCampSystem.Init();
        ...
        public void Update()
        {
            ...
            mStageSystem.Update();
            if(mCharacterSystem.result==-1)
            {
                result = -1;
            }
            else
            {
                result = mStageSystem.result;
            }
